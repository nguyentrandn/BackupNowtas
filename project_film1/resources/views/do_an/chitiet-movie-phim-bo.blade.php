...
<div class="main">
	<div class="colorbg"style="background-image:url({{URL::To('storage/'.$detail -> Film_bgr)}})" width="100%">
		<div class="mainn">
				<div class="mainleft">
					<div class="img-movie">
						<a href=""><img src="{{URL::To('storage/'.$detail -> Film_img)}}"  alt="" height="500px"></a>
					</div>
				...
				<div class="mainright">
					<div class="container">
						...
						<h1 id="title">{{ $detail->Film_name }}</h1>
						<a class="icon1" href=""><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></a>
								...
								<a href="{{route('watch-le',$detail->id)}}"><button class='button-watch'>Xem ngay</button>		</a>				
								</div>
						<div class="information">
							<ul>
								<li>Thời lượng: {{ $detail->Film_time }} </li>
								<li>Số tập: {{ $detail->Film_number }}</li>
								<li>Quốc gia: {{ $detail->country}} </li>
								<li>Năm xuất bản: {{ $detail->Film_year }}</li>
								<li>Ngày công chiếu: {{ $detail->Film_release }} </li>
								<li>Thể loại: {{ $detail->Film_category }} </li>
								<li>Đạo diễn: {{ $detail->Film_director }} </li>
								<li>Diễn viên: {{ $detail->	Film_actor }}</li>
							</ul>
						</div>
	...
	<div class="content">
		<div class="content1">
			...
			<p>{{ $detail->Film_info }}	</p>
			<h1 class="title-content">Diễn viên chính trong phim {{ $detail->Film_name }}</h1>
			<p>{{ $detail->Main_actor}}</p>
			<h1 class="title-content" id="about">Official trailer:			</h1>
			<iframe width="800" height="448" src="{{$detail ->Film_vd}}" title="YouTube video player" 
		...
		<div class="move-film">
			...
			<div class="row">
			@foreach($detail_tuongtu as $sptt)																		
				<div class="rowmini">
					<div class="item-header">
					<a href="{{route('chitiet-movie-phim-le',$sptt->id)}}"><img src="{{URL::To('storage/'.$sptt -> Film_img)}}"  alt="" height="250px" width="180px"></a>
						<a class="icon1" href=""><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></a>
						<a class="icon2" href=""><i class="fas fa-star"></i></a>
					</div>
					<div class="item-body">
						<p class="item-title">{{$sptt ->Film_name}}</p>																						
					</div>
